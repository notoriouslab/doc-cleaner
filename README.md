...
| AI 結構化     | Gemini 雲端、Grok 雲端 或 Ollama 本地             |
...
# 或
# Grok 雲端不需要額外 SDK；只要設定 XAI_API_KEY
# 或
...
cp .env.example .env                      # 填入 GEMINI_API_KEY / XAI_API_KEY（依後端而定）
...
python cleaner.py [選項]
  ...
  --ai              gemini | grok | ollama | none（預設：config 裡的 backend 或 gemini）
